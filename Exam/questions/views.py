...
def view_questions(request):
    prof = request.user
    questions = Question_DB.objects.filter(professor=prof)
    
    if request.method == 'POST' :
        delete = request.POST.get('delete', False)
        Question_DB.objects.filter(professor=prof,qno=delete).delete()
        return render(request, 'exam/viewquestions.html', {
